...
class TestBullet(Scene):
    ...
    def construct(self):
        ...
        vqueue3 = VGroup()
        self.an = 0
        colors= itertools.cycle(self.CONFIG2['colors'])

        def update_bullet1(obj, dt):
            bullet1 = Bullet(color=next(colors)).scale(0.7).rotate(self.CONFIG2['time'] * 10 + self.an * 10)
    ...
    def getting_tex(self,mob):
        texto=Text('R♥F').add_updater(lambda m: m.move_to(mob.get_center()+np.array([0,.1,0])))
        texto.scale(.3)
        return texto